...
export default function EditarVeiculo() {
  ...
  const removeExistingImage = async (index: number) => {
    ...
    try {
      ...
      if (primaryImageUrl === imageToRemove) {
        setPrimaryImageUrl('')
        // --- CORREÇÃO DO NOME DO CAMPO (BUG 1) ---
        await VehicleService.updateVehicle(vehicleId, { primaryImageUrl: null })
        // --- FIM DA CORREÇÃO ---
      }
    } catch (error) { toast.error('Erro ao remover imagem.') }
  }

  const setPrimaryImage = async (imageUrl: string) => {
    try {
      // --- CORREÇÃO DO NOME DO CAMPO (BUG 1) ---
      await VehicleService.updateVehicle(vehicleId, { primary_image_url: imageUrl })
      // --- FIM DA CORREÇÃO ---
      setPrimaryImageUrl(imageUrl)
      toast.success('Imagem principal definida!')
    } catch (error) { toast.error('Erro ao definir imagem principal.') }
  }
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                      {formData.existingImages.map((image, index) => (
                        <div key={index} className="relative group">
                          <img src={`${image}?width=300&quality=75`} alt={`Imagem ${index + 1}`} className="w-full h-24 object-cover rounded-lg"/>
