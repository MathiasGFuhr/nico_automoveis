...
interface Vehicle {
  ...
  price: number
  mileage: number
  image: string // Esta deve ser a URL da imagem principal
  fuel: string
  transmission: string
}
...
export default function VehicleCard({
  ...
  const getOptimizedImageUrl = (url: string) => {
    ...
    if (url.includes('supabase.co')) {
      ...
      // Parâmetros para imagem que preenche o espaço
      params.set('width', '400')
      params.set('height', '300') 
      // 'cover' garante que a imagem preencha todo o espaço
  ...
  return (
    <Link href={`/veiculos/${vehicle.id}`} className="block">
      <div className="bg-white rounded-lg shadow-md overflow-hidden hover:shadow-primary-lg transition-all duration-300 border border-primary-100 hover:border-primary-300 vehicle-card cursor-pointer">
      ...
      <div className="w-full h-40 sm:h-48 relative overflow-hidden" style={{ padding: 0, margin: 0 }}>
        {vehicle.image ? (
          <Image
            ...
            onLoad={() =>
              console.log('🖼️ Imagem carregada:', vehicle.brand, vehicle.model)
            }
          />
        ) : (
